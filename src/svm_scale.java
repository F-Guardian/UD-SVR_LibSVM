...
class svm_scale
{
	...
	private StringBuffer scaleString = new StringBuffer();
	...
	private void output_target(double value)
	{
		...
		//保存到指定文件
		scaleString  = scaleString .append(value + " ");
	...
	private void run(String[] argv) throws IOException
	{
		...
		while(readline(fp) != null)
		{
			//scaleString.delete(0,scaleString.length()-1); 
			int next_index = 1;
			double target;
			double value;
	...
	public  void saveScaleData()
	{
		...
		try {
			fw = new FileWriter(data_filename + "-scale");
			bw = new BufferedWriter(fw);
			
			bw.write(scaleString.toString());
			bw.flush();
